  ...
  describe("hits", () => {
    ...
    it("should ignore binary files", async () => {
      ...
      try {
        ...
        execSync("git add test.dat", { cwd: dir });
        execSync('git commit -qam "binary file modified"', { cwd: dir });
    ...
    it("should have date property set to current time", async () => {
      ...
      try {
        ...
        expect(hits).toHaveLength(1);
        expect(hits[0].date.getTime()).toBeGreaterThanOrEqual(before.getTime());
        expect(hits[0].date.getTime()).toBeLessThanOrEqual(after.getTime());
      } finally {
        rmSync(dir, { recursive: true, force: true });
      }
    });
